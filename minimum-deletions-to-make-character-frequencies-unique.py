...
class Solution:
    ...
    def minDeletions(self, s: str) -> int:
        ...
        frequency_dict = Counter(list(s))  # O(n)
        frequency_val = list(frequency_dict.values())  # O(n)

        num_deletion = 0
        while True:  # O(n)
            frequency_val = [i for i in frequency_val if i != 0]  # O(n)
            overlap_dict = dict(Counter(frequency_val))  # O(n)
            overlap_val = overlap_dict.values()  # O(n)
